...
You also need to read in the path to your AOI, which should be stored in
a folder called "`AOI`" as a `.gpkg` or `.shp`.
...
is dropped by default as it slows the download process.
...
`cores` and `data.path` arguments. There is a resolution argument
(`res`) which you can specify in the `setup_CHM` and `setup_DEM`
functions - the default is set to 1m, 5m, 10m, and 20m for both.

```{r processing-batch-files}

preprocess.bat <- setup_preprocess(data.path = data.path, cores = cores)
# shell.exec(preprocess.bat)

chm.bat <- setup_CHM(data.path = data.path, cores = cores)
# shell.exec(chm.bat)

dem.bat <- setup_DEM(data.path = data.path, cores = cores)
# shell.exec(dem.bat)

metrics.bat <- setup_metrics(data.path = data.path, cores = cores)
shell.exec(metrics.bat)
```
